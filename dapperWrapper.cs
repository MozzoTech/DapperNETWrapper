    ...
    public class DAPPERWRAPPER : IDisposable
    {
        ...
        public List<T> execute_stored_procedure<T>(string stored_procedure_name)
        {
            ...
            try
            {
                ...
                if (dp.Length() > 0)
                {
                    list_object = sql_connection.Query<T>(stored_procedure_name, dp, commandType: CommandType.StoredProcedure).ToList();
                }
                else
                {
        ...
        public T execute_scalar_stored_procedure<T>(string stored_procedure_name)
        {
            T tempObject;
            try
            {
                ...
                if (dp.Length() > 0)
                {
                    tempObject = sql_connection.ExecuteScalar<T>(stored_procedure_name, dp, commandType: CommandType.StoredProcedure);
                }
                else
                {
                    tempObject = sql_connection.ExecuteScalar<T>(stored_procedure_name);
                }
            }
            catch (SqlException ex)
            {
